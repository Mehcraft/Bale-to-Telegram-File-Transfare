...
const BOT_TOKEN = "8735271401:AAF-bJ6oAXTQa7wg52ch3DsEqzZr7hCkSBE"; // Replace with your Telegram bot token
const BALE_BOT_TOKEN = "670103193:Zb8qeqNZH_mHMDO4dOchmp1WAd7TpBRm1Mo"; // Replace with your Bale bot token
const BOT_WEBHOOK = "/endpoint";

// User Mapping: Bale Sender → Telegram Recipient
// example: { "123456789": "987654321" } means Bale user with ID 123456789 will have their files sent to Telegram user with ID 987654321
const USER_MAPPING = {
  "1070794900": "5814798351",
};
...
async function handleFileTransfer(senderId, recipientId, document) {
  ...
  try {
    ...
    if (result.ok) {
      await sendMessage(senderId, null, '✅ File sent successfully to Telegram.');
    } else {
      await sendMessage(senderId, null, `❌ Error: ${result.description}`);
    }
  } catch (error) {
    console.error('Error transferring file:', error);
    await sendMessage(senderId, null, '❌ Error while transferring file to Telegram.');
  }
}
...
async function handlePhotoTransfer(senderId, recipientId, photos) {
  try {
    ...
    if (result.ok) {
      await sendMessage(senderId, null, '✅ Photo sent successfully to Telegram.');
    } else {
      await sendMessage(senderId, null, `❌ Error: ${result.description}`);
    }
  } catch (error) {
    console.error('Error transferring photo:', error);
    await sendMessage(senderId, null, '❌ Error while transferring photo to Telegram.');
  }
}
...
async function handleVideoTransfer(senderId, recipientId, video) {
  ...
  try {
    ...
    if (result.ok) {
      await sendMessage(senderId, null, '✅ Video sent successfully to Telegram.');
    } else {
      await sendMessage(senderId, null, `❌ Error: ${result.description}`);
    }
  } catch (error) {
    console.error('Error transferring video:', error);
    await sendMessage(senderId, null, '❌ Error while transferring video to Telegram.');
  }
}
...
async function handleAudioTransfer(senderId, recipientId, audio) {
  ...
  try {
    ...
    if (result.ok) {
      await sendMessage(senderId, null, '✅ Audio sent successfully to Telegram.');
    } else {
      await sendMessage(senderId, null, `❌ Error: ${result.description}`);
    }
  } catch (error) {
    console.error('Error transferring audio:', error);
    await sendMessage(senderId, null, '❌ Error while transferring audio to Telegram.');
  }
}
...
async function getFile(fileId) {
  const response = await fetch(`https://tapi.bale.ai/bot${BALE_BOT_TOKEN}/getFile?file_id=${fileId}`);
  const data = await response.json();
  return data.result;
}

async function downloadFile(filePath) {
  const response = await fetch(`https://tapi.bale.ai/bot${BALE_BOT_TOKEN}/${filePath}`);
